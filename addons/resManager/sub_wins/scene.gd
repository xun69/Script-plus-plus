...
@onready var editor_des_txt = %editorDesTxt
...
func save_groups_info():
	...
	myConfig.save_dic_as_config(tscn_groups_path,dict)
...
func _on_item_groups_item_clicked(path):
	var tscn:PackedScene = load(path)
	var node = tscn.instantiate()
	# 显示节点结构
	var node_struct = SceneTreePlugin.get_node_structure_dic(node)
	load_node_struct_to_tree(node_struct,scene_nodes_tree)
	# 显示编辑器描述
	editor_des_txt.text = node.editor_description


# ItemGroups - 项被双击时
func _on_item_groups_item_dbl_click(path):
	var face:EditorInterface = GDEditor.face
	face.open_scene_from_path(path)
