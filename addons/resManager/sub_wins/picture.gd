...
# 场景分组文件
const pic_groups_path = "res://addons/resManager/data/pics_groups.cfg" 

# =================================== 节点引用 ===================================
@onready var item_groups = %ItemGroups
@onready var pic_preview = %picPreview
...
func _ready():
	has_ready = true
# 添加场景分组列表
	pic_preview.custom_minimum_size.y = pic_preview.get_parent().size.y
	load_groups()
...
func load_groups():
	if FileAccess.file_exists(pic_groups_path):
		var dict = myConfig.load_config_as_dic(pic_groups_path)
...
func save_groups_info():
	...
	myConfig.save_dic_as_config(pic_groups_path,dict)
...
func _on_item_groups_item_clicked(path):
	# 显示节点结构
	pic_preview.texture = load(path)


# ItemGroups - 项被双击时
func _on_item_groups_item_dbl_click(path):
	myFile.shell_open(path)
